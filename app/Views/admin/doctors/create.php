<div class="card max-w-3xl mx-auto">
      <div class="card-body">
        <form action="<?= $baseUrl ?>/admin/doctors/store" method="POST">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="doctor-name" class="form-control" required placeholder="Ej: Dr. Carlos Mendoza" autocapitalize="words">
                </div>
                <div class="form-group">
                    <label class="form-label">Cédula / Identificación *</label>
                    <input type="text" name="id_number" class="form-control" required maxlength="10" inputmode="numeric" placeholder="Ej: 1712345678">
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Teléfono</label>
                    <input type="text" name="phone" class="form-control" maxlength="15" inputmode="numeric" placeholder="Ej: 0991234567">
                </div>
                <div class="form-group">
                    <label class="form-label">Dirección / Consultorio</label>
                    <input type="text" name="address" id="doctor-address" class="form-control" placeholder="Ej: Av. Mariana de Jesús y Occidental" autocapitalize="sentences">
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group d-flex align-center mt-3">
                    <label class="form-label mb-0 d-flex align-center gap-1" style="cursor:pointer;">
                        <input type="checkbox" name="show_fee" value="1" checked style="width: 18px; height: 18px;">
            ...
            <div class="form-row">
                <div class="form-group" style="width: 100%;">
                    ...
                    <input type="email" name="email" id="doctor-email" class="form-control" placeholder="ejemplo@correo.com">
                    <small class="text-muted">Si tiene correo, se enviará un enlace para establecer su contraseña.</small>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group" style="width: 100%;">
                    <label class="form-label mb-0 d-flex align-center gap-1" style="cursor:pointer;">
                        <input type="checkbox" name="generate_credentials" id="generate-credentials" value="1" style="width: 18px; height: 18px;">
                        Generar nombre de usuario y contraseña temporal (si no tiene correo)
                    </label>
                    <small class="text-muted" id="credentials-hint" style="display:none; margin-top: 4px;">
            ...
            <div class="text-right mt-3 d-flex justify-content-end align-items-center gap-2">
                ...
                <button type="submit" class="btn btn-primary btn-sm"><i class="fa-solid fa-save"></i> Guardar Médico</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') closeNewSpecialtyModal();
});
function saveNewSpecialty() {
    ...
    .finally(() => {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-save"></i> Guardar Especialidad';
    });
}
/* ─── Toggle Generar Credenciales ─────────────────────────────────── */
const genCb = document.getElementById('generate-credentials');
const emailInput = document.getElementById('doctor-email');
const credHint = document.getElementById('credentials-hint');
if (genCb) {
    genCb.addEventListener('change', function() {
        credHint.style.display = this.checked ? 'block' : 'none';
    });
    // Auto-check si el email está vacío al enviar
    emailInput.addEventListener('input', function() {
        if (this.value.trim() === '') {
            genCb.checked = false;
            credHint.style.display = 'none';
        }
    });
}
