<div class="card mb-4">
    <div class="card-header d-flex justify-between align-center">
        ...
        <a href="<?= $baseUrl ?>/admin/<?= htmlspecialchars($returnUrl ?? 'appointments') ?>" class="btn btn-sm btn-secondary">Cancelar y Volver</a>
    </div>
    <div class="card-body">
        <div class="alert alert-info">
            <strong>Cita Actual:</strong><br>
            Paciente: <?= htmlspecialchars($appointment['patient_name']) ?><br>
            Médico: Dr. <?= htmlspecialchars($appointment['doctor_name']) ?> (<?= htmlspecialchars($appointment['specialty_name']) ?>)<br>
            Fecha original: <strong><?= date('d/m/Y H:i', strtotime($appointment['appointment_date'])) ?></strong>
        </div>
        <div class="alert alert-warning mt-3">
            <i class="fa-solid fa-circle-info"></i> Seleccione un nuevo día en el calendario para ver los médicos y turnos disponibles para <strong><?= htmlspecialchars($appointment['specialty_name'] ?? 'esta especialidad') ?></strong>.
        </div>
    </div>
</div>
<div class="card p-0">
...
<form id="rescheduleForm" action="<?= $baseUrl ?>/admin/appointments/update-schedule/<?= $appointment['id'] ?>" method="POST" style="display: none;">
    ...
    <input type="hidden" name="return_url" value="<?= htmlspecialchars($returnUrl ?? 'appointments') ?>">
</form>
<script>
    ...
    function selectSlot(doctorId, slotId, dateStr) {
        if (confirm('¿Estás seguro de que deseas reagendar la cita para este nuevo horario?')) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Lo sentimos, este turno ya no está disponible o acaba de ser tomado.');
                    openModal(dateStr); 
                }
            })
            .catch(err => {
