<div class="card" style="max-width: 600px; margin: 0 auto;">
    <div class="card-header">
        <h3><i class="fa-solid fa-user-doctor text-primary"></i> Mi Perfil</h3>
    </div>
    <div class="card-body">
        <form action="<?= $baseUrl ?>/doctor/profile/update" method="POST">
            <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">
            <div class="form-group mb-3">
                <label class="form-label">Nombre Completo</label>
                <div class="input-icon">
                    <i class="fa-solid fa-user"></i>
                    <input type="text" class="form-control" name="name" value="<?= htmlspecialchars($doctor['name'] ?? '') ?>" required>
                </div>
            </div>
            <div class="form-group mb-3">
                ...
                <div class="input-icon">
                    ...
                    <input type="text" class="form-control" value="<?= htmlspecialchars($doctor['id_number'] ?? '') ?>" disabled readonly style="opacity: 0.7;">
                </div>
                <small class="text-muted">El documento de identidad no puede ser modificado.</small>
            </div>
            <div class="form-group mb-3">
                <label class="form-label">Teléfono</label>
                <div class="input-icon">
                    <i class="fa-solid fa-phone"></i>
                    <input type="text" class="form-control" name="phone" value="<?= htmlspecialchars($doctor['phone'] ?? '') ?>">
                </div>
            </div>
            <div class="form-group mb-3">
                ...
                <div class="input-icon">
                    ...
                    <input type="text" class="form-control" name="address" value="<?= htmlspecialchars($doctor['address'] ?? '') ?>">
                </div>
            </div>
            <div class="form-group mb-3">
                ...
                <div class="input-icon">
                    ...
                    <input type="text" class="form-control" value="<?= htmlspecialchars($doctor['username'] ?? $doctor['nombre_usuario'] ?? '') ?>" disabled readonly style="background: var(--bg-surface, #f8fafc); font-weight: 600; opacity: 0.9;">
                </div>
            ...
            <div class="form-group mb-4" style="background: var(--bg-surface, #f8fafc); padding: 1.25rem; border-radius: 8px; border: 1px solid var(--border-color, #e2e8f0);">
                <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem;">
                    ...
                    <div style="padding-top: 2px;">
                        <input type="checkbox" name="two_factor_enabled" id="two_factor_enabled" value="1" <?= !empty($doctor['two_factor_enabled']) ? 'checked' : '' ?> style="width: 1.35rem; height: 1.35rem; cursor: pointer;">
                    </div>
                </div>
            </div>
