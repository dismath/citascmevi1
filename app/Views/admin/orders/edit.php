...
<div class="card">
    <div class="card-header d-flex justify-between align-center">
        <h2><i class="fa-solid fa-pen"></i> Editar Orden / Prefactura #<?= str_pad($order['id'], 6, '0', STR_PAD_LEFT) ?></h2>
        ...
        <div class="order-create-grid">
            <div>
                <div class="order-info-card">
                    ...
                    <p><strong>Nombre:</strong> <?= htmlspecialchars($order['patient_name']) ?></p>
                    <p><strong>CI / RUC:</strong> <?= htmlspecialchars($order['patient_id_number']) ?></p>
                    <p><strong>Email:</strong> <?= htmlspecialchars($order['patient_email'] ?? '—') ?></p>
                </div>
                <div class="order-info-card" style="margin-top: 1.5rem;">
                    <h4><i class="fa-regular fa-calendar-check"></i> Datos de la Orden</h4>
                    <hr style="margin: 0.5rem 0;">
                    <p><strong>Nro. de Orden:</strong><br>
                        <span class="order-num-badge">#<?= str_pad($order['id'], 6, '0', STR_PAD_LEFT) ?></span>
                    </p>
                    <p style="margin-top: 0.6rem;"><strong>Especialidad:</strong> <?= htmlspecialchars($order['specialty_name']) ?></p>
                    <p><strong>Fecha Generación:</strong> <?= date('d/m/Y H:i', strtotime($order['created_at'])) ?></p>
                </div>
            </div>
            <div class="order-form-panel">
                <form action="<?= $baseUrl ?>/admin/orders/update/<?= $order['id'] ?>" method="POST" id="orderForm">
...
document.addEventListener('DOMContentLoaded', function () {
    loadCatalogItems(<?= (int)$order['appointment_id'] ?>);
    renderOrderItems();
});
